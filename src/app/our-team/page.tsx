...
async function getData() {
  ...
  const res = await fetch(url, { cache: "no-store" });
  if (!res.ok) throw new Error("Failed to fetch Fee Management page data");

  const all: ApiResponseItem[] = await res.json();

  return {
    OurTeam: all.find((s: ApiResponseItem) => s.component === "TeamSection")?.contents || []
  };
}
...
const page = async() => {
  ...
  return (
    <>
      <TeamSection contents={data.OurTeam ?? []}/>
    </>
  )
}
